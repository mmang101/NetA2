...
// These are seq numbers used by calling and receiving application layers
...
//  Do our initialization and any requested by student's Transport Layer 
    ...
    *************************************************************************/
    while (TRUE) {
        eventptr = evlist;        // get next event to simulate from Q headA
	...
	// Print out what's on the Q
        ...
        *********************************************************************/
        if ( eventptr->evtype == FROM_LAYER5 ) {
            // Use the seq number as starter for the message string
...
void   GetMessageString( int entity, int  SeqNum, char *ReturnString ) {
    ...
    // Produce a seq that MAY defeat the checksum
    if ( SeqNum % 7 == 0 )   {
        ReturnString[0] = 100;
        ReturnString[12] = 50;
    }
    ...
    // Now we embed the seq number in the string
...
void tolayer5( int AorB, struct msg message) {
    ...
    // We REALLY shouldn't increment the next expected seq number, BUT
